...
class RandomSensorDrop_RGBIR:
    ...
    def __call__(self, sample):
        if random.random() > self.p_drop:
            return sample  # Keep all channels
    ...
    def get_modality_mask(self, sample):
        """Return [rgb_available, ir_available] binary mask."""
        is_tensor = isinstance(sample, torch.Tensor)
        if is_tensor:
...
class DataAugmentationDINO_RGBIR:
    ...
    def __init__(self, global_crop_size=224, local_crop_size=96,
        ...
        self.global_post1 = [to_tensor, normalize, self.sensor_drop]
        ...
        self.global_post2 = [to_tensor, normalize, self.sensor_drop]
        self.blur2_p = 0.1
        ...
        self.local_post = [to_tensor, normalize, self.sensor_drop]
    ...
    def __call__(self, image):
        ...
        g1 = self._apply_post(g1, self.global_post1, blur_p=1.0)
        crops.append(g1)

        # Global crop 2
        g2 = self.global_transfo2(image)
        g2 = self._apply_post(g2, self.global_post2,
        ...
        for _ in range(self.local_crops_number):
            ...
            lc = self._apply_post(lc, self.local_post, blur_p=self.blur_local_p)
            crops.append(lc)
