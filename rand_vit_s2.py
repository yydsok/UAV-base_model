...
def _require_bigearthnet_runtime_deps():
    if LMDBDataset is None or random_subset is None:
        raise ImportError(
...
def eval_linear(args):
    ...
    if args.arch in vits.__dict__.keys():
        model = vits.__dict__[args.arch](patch_size=args.patch_size, num_classes=0, in_chans=12)
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, _, target) in metric_logger.log_every(loader, 20, header):
        # move to gpu
...
def validate_network(val_loader, model, linear_classifier, n, avgpool):
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    for (inp, _, target) in metric_logger.log_every(val_loader, 200, header):
        # move to gpu
        inp = inp.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
