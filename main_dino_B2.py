...
try:
    ...
    _CVTRANSFORMS_IMPORT_ERROR = None
except ModuleNotFoundError as exc:
    cvtransforms = None
    _CVTRANSFORMS_IMPORT_ERROR = exc
from models.rs_transforms import RandomChannelDrop, GaussianBlur, Solarize, RandomBrightness, RandomContrast, ToGray, RandomSensorDrop, SensorDrop
### end of change ###
import pdb
...
def _require_bigearthnet_runtime_deps():
    if LMDBDataset is None:
        raise ImportError(
            "LMDBDataset import failed. Install 'lmdb' to use BigEarthNet LMDB data."
        ) from _LMDB_IMPORT_ERROR
    if cvtransforms is None:
        raise ImportError(
            "cvtorchvision import failed. Install 'opencv-torchvision-transforms-yuzhiyang'."
        ) from _CVTRANSFORMS_IMPORT_ERROR
...
def get_args_parser():
    ...
    # new
    parser.add_argument('--lmdb_dir', default='/path/to/imagenet/', type=str,
        help='Please specify path to the ImageNet folder.')
    parser.add_argument('--bands', type=str, default='B2',
        choices=['RGB', 'B12', 'B2', 'B14'], help="input bands")
...
def train_dino(args):
    _require_bigearthnet_runtime_deps()
    ...
    '''
    transform_S2 = DataAugmentationDINO_S2(
        args.global_crops_scale,
        args.local_crops_scale,
        args.local_crops_number,
    )
    transform_S1 = DataAugmentationDINO_S2(
        args.global_crops_scale,
        args.local_crops_scale,
        args.local_crops_number,
    )
    ...
    if args.lmdb:
        ...
        raise ValueError("This script currently supports only --lmdb data loading.")
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images_s12,_) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        
        #images = [torch.cat((images_s2[i],images_s1[i]),axis=1) for i in range(len(images_s2))]
        images = [images_s12[i][:,12:,:,:] for i in range(len(images_s12))]
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        #sensor_drop = cvtransforms.RandomApply([RandomSensorDrop()], p=0.5)
        s2_drop = cvtransforms.Compose([SensorDrop('S2')])
